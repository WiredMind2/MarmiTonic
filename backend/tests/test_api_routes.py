...
class TestIngredientsEndpoints:
    ...
    @patch('backend.routes.ingredients.IngredientService')
    def test_get_all_ingredients(self, mock_service_class, client, mock_ingredient):
        """Test GET /ingredients/ingredients"""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.get_all_ingredients.return_value = [mock_ingredient]

        # Note: The router uses a module-level service instance
        with patch('backend.routes.ingredients.service', mock_service):
    ...
    def test_get_all_ingredients_error(self, mock_service, client):
        """Test GET /ingredients/ingredients with error"""
        mock_service.get_all_ingredients.side_effect = Exception("Database error")
    ...
    def test_search_ingredients(self, mock_service, client, mock_ingredient):
        """Test GET /ingredients/ingredients/search?q=query"""
    ...
    def test_search_ingredients_no_query(self, mock_service, client):
        """Test GET /ingredients/ingredients/search without query parameter"""
    ...
    def test_search_ingredients_empty_result(self, mock_service, client):
        """Test GET /ingredients/ingredients/search with no results"""
        mock_service.search_ingredients.return_value = []

        response = client.get("/ingredients/ingredients/search?q=NonExistent")
    ...
    def test_update_inventory(self, mock_service, client):
        """Test POST /ingredients/ingredients/inventory"""
    ...
    def test_update_inventory_empty_list(self, mock_service, client):
        """Test POST /ingredients/ingredients/inventory with empty ingredients"""
        mock_service.update_inventory.return_value = None
    ...
    def test_update_inventory_error(self, mock_service, client):
        """Test POST /ingredients/ingredients/inventory with error"""
    ...
    def test_update_inventory_invalid_payload(self, mock_service, client):
        """Test POST /ingredients/ingredients/inventory with invalid payload"""
        payload = {
            "user_id": "user123"
    ...
    def test_get_inventory(self, mock_service, client):
        """Test GET /ingredients/ingredients/inventory/{user_id}"""
    ...
    def test_get_inventory_empty(self, mock_service, client):
        """Test GET /ingredients/ingredients/inventory/{user_id} with empty inventory"""
    ...
    def test_get_inventory_error(self, mock_service, client):
        """Test GET /ingredients/ingredients/inventory/{user_id} with error"""
...
class TestPlannerEndpoints:
    ...
    @patch('backend.routes.planner.PlannerService')
    def test_party_mode(self, mock_service_class, client):
        """Test POST /planner/planner/party-mode"""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
        mock_service.optimize_party_mode.return_value = {
            "selected_ingredients": ["Rum", "Vodka"],
            "covered_cocktails": ["Mojito", "Moscow Mule"]
        }

        # Note: The router uses a module-level service instance
        with patch('backend.routes.planner.service', mock_service):
            payload = {"num_ingredients": 2}
            response = client.post("/planner/planner/party-mode", json=payload)
        
            assert response.status_code == 200
            data = response.json()
            assert len(data["selected_ingredients"]) == 2
            assert len(data["covered_cocktails"]) == 2

    @patch('backend.routes.planner.service')
    def test_party_mode_zero_ingredients(self, mock_service, client):
        """Test POST /planner/planner/party-mode with zero ingredients"""
    ...
    @patch('backend.routes.planner.service')
    def test_party_mode_negative_ingredients(self, mock_service, client):
        """Test POST /planner/planner/party-mode with negative ingredients"""
    ...
    @patch('backend.routes.planner.service')
    def test_party_mode_error(self, mock_service, client):
        """Test POST /planner/planner/party-mode with error"""
        mock_service.optimize_party_mode.side_effect = Exception("Optimization failed")

        payload = {"num_ingredients": 5}
        response = client.post("/planner/planner/party-mode", json=payload)
        
        assert response.status_code == 500
        assert "Failed to optimize party mode" in response.json()["detail"]

    @patch('backend.routes.planner.service')
    def test_playlist_mode(self, mock_service, client):
        """Test POST /planner/planner/playlist-mode"""
    ...
    @patch('backend.routes.planner.service')
    def test_playlist_mode_empty_list(self, mock_service, client):
        """Test POST /planner/planner/playlist-mode with empty list"""
    ...
    def test_playlist_mode_error(self, mock_service, client):
        """Test POST /planner/planner/playlist-mode with error"""
    ...
    @patch('backend.routes.planner.service')
    def test_union_ingredients(self, mock_service, client):
        """Test POST /planner/planner/union-ingredients"""
        mock_service.get_union_ingredients.return_value = ["Rum", "Lime", "Mint", "Sugar"]
    ...
    @patch('backend.routes.planner.service')
    def test_union_ingredients_empty_list(self, mock_service, client):
        """Test POST /planner/planner/union-ingredients with empty list"""
        payload = {"cocktail_names": []}
        response = client.post("/planner/planner/union-ingredients", json=payload)
    ...
    @patch('backend.routes.planner.service')
    def test_union_ingredients_not_found(self, mock_service, client):
        """Test POST /planner/planner/union-ingredients with non-existent cocktails"""
        mock_service.get_union_ingredients.side_effect = ValueError("No valid cocktails found")

        payload = {"cocktail_names": ["NonExistent"]}
        response = client.post("/planner/planner/union-ingredients", json=payload)
        
        assert response.status_code == 400
        assert "No valid cocktails found" in response.json()["detail"]

    @patch('backend.routes.planner.service')
    def test_union_ingredients_error(self, mock_service, client):
        """Test POST /planner/planner/union-ingredients with error"""
        mock_service.get_union_ingredients.side_effect = Exception("Database error")

        payload = {"cocktail_names": ["Mojito"]}
        response = client.post("/planner/planner/union-ingredients", json=payload)
        
        assert response.status_code == 500
        assert "Failed to get union ingredients" in response.json()["detail"]
...
class TestInsightsEndpoints:
    ...
    def test_get_graph_analysis(self, mock_service_class, client):
        """Test GET /insights/insights/graph"""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
    ...
    def test_get_graph_analysis_error(self, mock_service_class, client):
        """Test GET /insights/insights/graph with error"""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
    ...
    def test_get_graph_visualization(self, mock_service_class, client):
        """Test GET /insights/insights/visualization"""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
    ...
    def test_get_graph_visualization_error(self, mock_service_class, client):
        """Test GET /insights/insights/visualization with error"""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
    ...
    def test_export_graph(self, mock_service_class, client):
        """Test GET /insights/insights/export"""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
    ...
    def test_export_graph_error(self, mock_service_class, client):
        """Test GET /insights/insights/export with error"""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
    ...
    def test_get_disjoint_components(self, mock_service_class, client):
        """Test GET /insights/insights/components"""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
    ...
    def test_get_disjoint_components_error(self, mock_service_class, client):
        """Test GET /insights/insights/components with error"""
        mock_service = Mock()
        mock_service_class.return_value = mock_service
